...
struct BFH
{
    ...
    unsigned int   bfOffBits;        /* Offset to bitmap data(смещение до пол€ данных, обычно 54 = 16 + biSize )*/
} bh;
#pragma pack()
struct BIH
{
    ...
    unsigned int   biSizeImage;      /* Size of image data (количество байт в поле данных, обычно устанавливаетс€ в 0)*/
...
RGBTRIPLE* loadBMP( const char *fname, int &mx, int &my ) //функци€ загрузки файла
{
    mx=my=-1;
    ...
    int filesize = ftell(f);//определ€ем текущую позицию в файле
...
bool saveBMP(const char *fname, RGBTRIPLE *v, int mx, int my) //функци€ сохранени€
{
    bih.biWidth=mx;
    bih.biHeight=my;
...
RGBTRIPLE* ex(RGBTRIPLE *v, int &mx, int &my, float x1, float y1) //пот€нуть по х и у
{
    int nmx, nmy;
    int k=0,k1=0;
    nmx=round(mx*x1);
...
RGBTRIPLE* matrixConvolution(RGBTRIPLE *v, int mx, int my, int matrix[3][3], int div) //маьрица свЄртки
{
    RGBTRIPLE neighbors[3][3];
    RGBTRIPLE* v1 = new RGBTRIPLE[mx*my];
    RGBTRIPLE* ptr1 = v1;
    int y;
// дл€ omp - #pragma omp parallel for private(y), private(neighbors)+
    for(y = 0; y < my; y++)
    {
        for(int x = 0; x < mx; x++)
        {
            int rgbtRed = 0, rgbtGreen = 0, rgbtBlue = 0;
            // вычисл€ем новое значение, использу€ соседей... учитываем, что строки идут снизу вверх!
...
RGBTRIPLE* matrixConvolution1(RGBTRIPLE *v, int mx, int my, double matrix[3][3], int div) //маьрица свЄртки
{
    RGBTRIPLE neighbors[3][3];
    RGBTRIPLE* v1 = new RGBTRIPLE[mx*my];
    RGBTRIPLE* ptr1 = v1;
